...
def fetch_and_save_data(year, variant, geeklist_id):
    """
    Fetches data from the BGG API with increasing backoff retries and saves 
    the XML response only upon full success.
    
    Args:
        year: The year for the VFM (e.g., 22, 23, 24, 25)
        variant: The variant type (bgg, dte, dtw, spr)
        geeklist_id: The BGG geeklist ID to fetch
    """
    BGG_API_URL = f"https://boardgamegeek.com/xmlapi/geeklist/{geeklist_id}?comments=1"
    OUTPUT_FILENAME = f"vfm{year}{variant}.xml"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Fetch BGG VFM Geeklist data')
    parser.add_argument('--year', required=True, help='VFM year (e.g., 22, 23, 24, 25)')
    parser.add_argument('--variant', required=True, help='VFM variant (bgg, dte, dtw, spr)')
    parser.add_argument('--geeklist', required=True, help='BGG geeklist ID')
    
    args = parser.parse_args()
    
    fetch_and_save_data(args.year, args.variant, args.geeklist)
