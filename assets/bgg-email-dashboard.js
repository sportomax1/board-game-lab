(() => {
  ...
  let emails=[], loaded=false, selected=null, direction='all', query='', actionFilter='all', emailLimit=60;
  ...
  function normSubject(v){return String(v||'(No subject)').replace(/^\s*((re|fw|fwd):\s*)+/ig,'').trim().toLowerCase();}
  function linkedContact(r){const hay=(sender(r)+','+recipients(r)).toLowerCase();return (window.contacts||[]).some(c=>c.email&&hay.includes(String(c.email).toLowerCase()));}
  function linkedGame(r){const hay=(String(r.subject||'')+' '+String(body(r)||'')).toLowerCase();return (window.pipelineGames||[]).some(g=>g.name&&hay.includes(String(g.name).toLowerCase()));}
  function threads(){
    const m=new Map();emails.forEach(r=>{const k=normSubject(r.subject),a=m.get(k)||[];a.push(r);m.set(k,a)});
    return [...m.values()].map(a=>{a.sort((x,y)=>new Date(when(y)||0)-new Date(when(x)||0));const latest=a[0];return {latest,messages:a,count:a.length,needsReply:dir(latest)==='received',hasContact:a.some(linkedContact),hasGame:a.some(linkedGame)}}).sort((a,b)=>new Date(when(b.latest)||0)-new Date(when(a.latest)||0));
  }
  function filtered(){const q=query.trim().toLowerCase();return threads().filter(t=>{const r=t.latest;if(direction!=='all'&&dir(r)!==direction)return false;if(actionFilter==='reply'&&!t.needsReply)return false;if(actionFilter==='game'&&!t.hasGame)return false;if(actionFilter==='contact'&&!t.hasContact)return false;return !q||t.messages.some(x=>[x.subject,sender(x),recipients(x),body(x)].some(v=>String(v||'').toLowerCase().includes(q)))})}
  ...
  function render(){const host=document.getElementById('emailDashboard'),s=stats(),threadsFiltered=filtered(),rows=threadsFiltered.slice(0,emailLimit);host.innerHTML=`
    ...
    <div class="emailToolbar"><div class="emailSearch">${icon('fa-magnifying-glass')}<input id="emailSearch" value="${esc(query)}" placeholder="Search conversations"></div><div class="emailFilters"><button data-dir="all">All</button><button data-dir="received">Received</button><button data-dir="sent">Sent</button></div><div class="emailFilters emailActionFilters"><button data-action="all">Any</button><button data-action="reply">Needs reply</button><button data-action="game">Linked game</button><button data-action="contact">Linked contact</button></div><span class="emailResultCount">${threadsFiltered.length} conversation${threadsFiltered.length===1?'':'s'}</span></div>
    <div class="emailSplit"><div class="emailList">${rows.map(threadRow).join('')||'<div class="emailEmpty">No conversations match these filters.</div>'}${threadsFiltered.length>rows.length?'<button id="emailMore" class="showMore">Show more <span>'+rows.length+' of '+threadsFiltered.length+'</span></button>':''}</div><aside id="emailDetail" class="emailDetail">${selected?detail(selected):emptyDetail()}</aside></div>`;
    host.querySelectorAll('[data-dir]').forEach(b=>{b.classList.toggle('active',b.dataset.dir===direction);b.onclick=()=>{direction=b.dataset.dir;emailLimit=60;render();};});
    host.querySelectorAll('[data-action]').forEach(b=>{b.classList.toggle('active',b.dataset.action===actionFilter);b.onclick=()=>{actionFilter=b.dataset.action;emailLimit=60;render();};});
    document.getElementById('emailSearch').oninput=e=>{query=e.target.value;emailLimit=60;clearTimeout(window.__emailSearchT);window.__emailSearchT=setTimeout(render,140);};
    document.getElementById('emailMore')?.addEventListener('click',()=>{emailLimit+=60;render()});
  ...
  function threadRow(t){const r=t.latest,id=String(r.id||r.outlook_message_id||r.message_id||''),d=dir(r),other=d==='sent'?recipients(r):sender(r),preview=String(body(r)).replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim();return `<article class="emailRow ${selected&&String(selected.id||selected.outlook_message_id||selected.message_id)===id?'selected':''}" data-email-id="${esc(id)}"><div class="emailDirection ${d}">${icon(d==='sent'?'fa-arrow-up-right-from-square':'fa-arrow-down')}</div><div class="emailRowMain"><div class="emailRowTop"><strong>${esc(other||'Unknown')}</strong><time>${fmt(when(r))}</time></div><h4>${esc(r.subject||'(No subject)')} ${t.count>1?'<span class="threadCount">'+t.count+'</span>':''}</h4><p>${esc(preview||'No archived preview') }</p><div class="emailRowFlags">${t.needsReply?'<span>Needs reply</span>':''}${t.hasGame?'<span>Linked game</span>':''}${t.hasContact?'<span>Linked contact</span>':''}</div></div></article>`;}
