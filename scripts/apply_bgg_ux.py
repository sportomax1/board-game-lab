...
p=Path('bgg-contacts.html'); bga=Path('bga.html'); s=p.read_text(); b=bga.read_text()
...
if 'cdn.tailwindcss.com' not in s:s=s.replace('<style>','<script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><style>',1)
body_match=re.search(r'<body[^>]*>(.*?)</body>',b,re.S|re.I); body=body_match.group(1) if body_match else b
inline_scripts=re.findall(r'<script(?![^>]*\b(?:src|type)\s*=)[^>]*>(.*?)</script>',body,re.S|re.I); bga_js='\n'.join(inline_scripts)
